...
mu = G * (M + m); % 
[t, y] = ode45(@(t,y) eom_fun(t,y,F), [0 10*60*60], [Ro; 0; 0;0; sqrt(mu/Ro); 0], odeoptions);

plot(t,y(:, 1:3));
title('State Vector Plot Against Time');
legend({'$x$', '$y$', '$z$', '$\dot{x}$',  '$\dot{y}$',  '$\dot{z}$'}, 'Interpreter', 'latex', 'FontSize', 15);
xlabel('t (s)');
ylabel('State Vector');
